...
On crée un template de fonction avec un booléen comme paramètre, afin de générer une fonction différente (et donc plus efficiente) selon si le booléen est à true ou false.
...
le constructeur à 3 arguments est appelé sur une array de capacité 2, ce qui engendre une erreur lorsque qu'on tente d'insérer les 3 éléments dans l'array. En effet, on a mis aucun protection contre ce cas de figure. Une solution consisterait à faire un assert.
...
On va se retrouver avec une troisième valeur z par défaut.
...
Le constructeur avec paramètre variadique ne prend pas en paramètre les objets du conteneur, mais le conteneur en lui-même. Le size of renvoie donc toujours 1 et l'assertion échoue. Pour y remédier, on peut forcer la conversion en plaçant un paramètre de type t en premier.
